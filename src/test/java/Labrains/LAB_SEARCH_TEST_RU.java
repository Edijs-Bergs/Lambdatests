...
public class LAB_SEARCH_TEST_RU {
    ...
    public void setUp() throws Exception {
        ...
        capabilities.setCapability("name", "LAB_SEARCH_TEST_RU");
    ...
    @Test()
    public void LAB_SEARCH_TEST_RU(){
        try {
            ...
            Serch.sendKeys("ЭКО"); //Send string Eko Somiņa
            ...
            String Substring = "search_product_title=%D0%AD%D0%9A%D0%9E"; //Comparison string
            boolean result = page_url.contains(Substring); //Determine page
            System.out.println(result); //print value
            if (result)
            {
                WebElement Search_Product = driver.findElement(By.xpath("//a[contains(text(),'ЭКО косметичка')]")); // Search for product that contains
                String Product = Search_Product.getText(); // Get the value of the
                System.out.println(Product); //Print product string

                String Product_Name = "ЭКО косметичка";
