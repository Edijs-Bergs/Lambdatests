...
public class LAB_SEARCH_TEST_LV {
    ...
    @Test()
    public void LAB_SEARCH_TEST_LV(){
        try {
            ...
            Serch.sendKeys("Eko Somiņa"); //Send string Eko Somiņa
            ...
            String Substring = "search_product_title=Eko+Somi%C5%86a"; //Comparison string
            boolean result = page_url.contains(Substring); //Determine page
            System.out.println(result); //print value
            if (result)
            {
                WebElement Search_Product = driver.findElement(By.xpath("//a[contains(text(),'EKO kosmētikas somiņa')]")); // Search for product that contains
                String Product = Search_Product.getText(); // Get the value of the
                System.out.println(Product); //Print product string

                String Product_Name = "EKO kosmētikas somiņa";
