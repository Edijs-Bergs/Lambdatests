...
public class LAB_CATEGORY_TEST_RU {
    ...
    public void setUp() throws Exception {
        ...
        capabilities.setCapability("name", "LAB_CATEGORY_TEST_RU");
    ...
    @Test()
    public void LAB_CATEGORY_TEST_RU(){
        try {
            driver.get("https://labrains.eu/ru"); //Go to Store site
            ...
            WebElement move = driver.findElement(By.xpath("//a[contains(text(),'МАГАЗИН')]"));
            ...
            if (result) {
                ...
                if (Element_On_Page)
                {
                    move = driver.findElement(By.xpath("//a[contains(text(),'МАГАЗИН')]"));
                    a.moveToElement(move).perform();
                    wait = new WebDriverWait(driver,2); //pause
                    move = driver.findElement(By.xpath("//a[contains(text(),'Восстановление кожи')]"));
                    a.moveToElement(move).click().perform();

                    String page_url2 = driver.getCurrentUrl(); //Get currennt url
                    System.out.println(page_url2); //print url

                    String Substring2 = "vosstanovlenie-kozhi"; //Comparison string
                    boolean result2 = page_url2.contains(Substring2); //Determine page
                    System.out.println(result2); //print value
                    if (result2) {
                        WebElement Search_Product2 = driver.findElement(By.xpath("//h1[contains(text(),'Восстановление кожи')]"));
                        String Product2 = Search_Product2.getText(); // Get the value of the
                        System.out.println(Product2); //Print product string

                        String Test_String2 = "ВОССТАНОВЛЕНИЕ КОЖИ";
